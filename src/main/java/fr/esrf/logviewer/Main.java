...
public class Main extends JFrame {
    ...
    public Main(String[] aArgs, boolean closeOnExit) {
        ...
        splash.progress(splashProgression++);
        // Add control panel
        final ControlPanel controlPanel = new ControlPanel(model, this);
    ...
    */
    ...
    private void addCoLocatedSources() {
        ...
        if (n instanceof DeviceNode) {
            ...
            mLabel.setText("Adding  " + deviceName + " and colocated devices...");
            if (loggingReceiver.addCoLocatedSources(deviceName)!=-1) {
                mHistoryArea.write("Added  " + deviceName + " and colocated devices");
            }
        }
        mLabel.reset();
    }
    ...
    private void removeLoggingSource() {
        ...
        if (n instanceof DeviceNode) {
            String deviceName = ((DeviceNode) n).devName;
            mLabel.setText("Contacting " + deviceName + "...");
            if (loggingReceiver.removeLoggingSource(deviceName)!=-1) {
                mHistoryArea.write("Removed " + deviceName);
            }
        }
        mLabel.reset();
    }
    ...
    private void removeCoLocatedSources() {
        ...
        if (n instanceof DeviceNode) {
            String deviceName = ((DeviceNode) n).devName;
            mLabel.setText("Removing  " + deviceName + " and colocated devices...");
            if (loggingReceiver.removeCoLocatedSources(deviceName)!=-1) {
                mHistoryArea.write("Removed  " + deviceName + " and colocated devices");
            }
        }
        mLabel.reset();
    }
    ...
    private class SourceListActionListener implements ActionListener {
        public void actionPerformed(ActionEvent e) {
            ...
            final List<String> sources = loggingReceiver.getLoggingSources();
            if (sources.size()==0) {
                mHistoryArea.write("\t- none");
            } else {
                for (String source : sources) {
                    mHistoryArea.write("\t- " + source);
                }
            }
        }
    }
    ...
    private class LevelActionListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (n instanceof DeviceNode) {
                ...
                String devname = ((DeviceNode) n).devName;
                mLabel.setText("Contacting " + devname + "...");
                if (loggingReceiver.setDeviceLoggingLevel(devname, level, mAddBefore)!=-1) {
                    if (mAddBefore) {
                        mHistoryArea.write("Added " + devname);
                    }
